...
- **两个独立的中英开关**：系统输入源（`Ctrl+空格` / 🌐）和砚台模式（Shift）。
  系统键盘一旦是中文，按键在到达插件前就被吃掉，砚台**在任何模式下都不工作**。
...
- **提示文案要围绕「砚台还工作吗」，不是「你在哪个模式」**。用户需要知道工具灵不灵，
  不是自己处在哪一档。只有两种状态值得说：系统键盘是中文（砚台被架空）、
  系统键盘是英文（砚台正常，切模式不该夹带任何叮嘱）。
